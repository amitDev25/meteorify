...
document.getElementById("searchCity").onkeyup = function () {
    ...
    fetch(url).then((res) => {
        ...
        // console.log(data); 
        document.querySelector(".allInputs").style.display = "flex"
        document.querySelector(".allInputs").innerHTML = ""
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            console.log(data)
            document.querySelector(".allInputs").innerHTML += `<div class="searchCityResults" data-city="${CityName}" data-country="${countryName}">
            <span class="material-symbols-outlined locationIcon">location_on</span>
            <div class="searchAllCity">
                <p class="placeName">${CityName}</p>
                <p class="stateCountry">${stateName}, ${countryName}</p>
            </div>
        </div>
    `;
        }

        document.querySelectorAll(".searchCityResults").forEach(element => {
            element.addEventListener("click", () => {
                let CityName2 = element.getAttribute("data-city");
                let countryName2 = element.getAttribute("data-country");
                document.getElementById("searchCity").value = `${CityName2}, ${countryName2}`;
                document.querySelector(".allInputs").style.display = "none";
            });
        });
    });
